...
(0, command_1.command)({
    ...
    if (!utils_1.isUrl.tiktok(url)) {
        return msg.reply("_need a valid tiktok url_");
    }
    const resp = await axios_1.default.get(`https://kord-api.vercel.app/tik-img?url=${url}`);
...
(0, command_1.command)({
    ...
    const q = match?.trim();
    if (!q)
        return msg.reply("_provide app name or package_");
    const r = await axios_1.default.get(`https://kord-api.vercel.app/apk?q=${encodeURIComponent(q)}`);
    const d = r.data;
    if (!d?.download_url)
        return msg.reply("_no apk found_");
...
(0, command_1.command)({
    ...
    if (!url)
        return msg.reply("_send or reply with a valid Instagram link_");
    const r = await axios_1.default.get(`https://kord-api.vercel.app/insta?url=${encodeURIComponent(url)}`);
    const d = r.data;
    if (!d?.url)
        return msg.reply("_no media found_");
    await msg.client.sendMessage(msg.jid, {
        video: { url: d.url },
        mimetype: "video/mp4",
        caption: `\n📹 Type: ${d.type || "video"}`,
...
(0, command_1.command)({
    ...
    const url = found ? found[0] : null;
    if (!url)
        return msg.reply("_send or reply with a valid Pinterest link_");
    const r = await axios_1.default.get(`https://kord-api.vercel.app/pinterest?url=${encodeURIComponent(url)}`);
    const d = r.data?.data?.data;
    if (!d?.downloads?.[0]?.url)
        return msg.reply("_no media found_");
    await msg.client.sendMessage(msg.jid, {
...
(0, command_1.command)({
    ...
    if (!url)
        return msg.reply("_send or reply with a valid TikTok link_");
    const r = await axios_1.default.get(`https://kord-api.vercel.app/tiktok?url=${encodeURIComponent(url)}`);
    const d = r.data?.data;
    if (!d?.downloadLinks)
        return msg.reply("_no media found_");
